 ...
 * @param {Object} oldData - Previous scrape data
 * @param {Object} newData - New scrape data
 * @returns {Object} Differences object
 */
export function compareScrapes(oldData, newData) {
  if (!oldData || !newData) {
 ...
 * @param {Object} changes - Changes object from compareScrapes
 * @returns {Object} Summary object
 */
export function getChangeSummary(changes) {
  return {
    hasChanges: changes.statistics.totalChanges > 0,
    totalChanges: changes.statistics.totalChanges,
    additions: changes.statistics.additions,
    removals: changes.statistics.removals,
    modifications: changes.statistics.modifications,
    summary: {
      linksChanged: (changes.added.links?.length || 0) + (changes.removed.links?.length || 0) > 0,
      imagesChanged: (changes.added.images?.length || 0) + (changes.removed.images?.length || 0) > 0,
      titleChanged: !!changes.modified.title,
      textChanged: !!changes.modified.text,
      metaChanged: (changes.added.metaTags?.length || 0) + (changes.removed.metaTags?.length || 0) > 0
    }
  };
}
